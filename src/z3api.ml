 ...
 *                                                           *
 * check_sat_sigma : sigma -> bool                           *
 * string_of_sigma : sigma -> string                         *
 * is_sigma_empty  : sigma -> bool                           *
 * prop_of_sigma   : sigma -> prop                           *
 * --more--                                                  *
 *                                                           *
 * CONSTANTS:                                                *
 * ----------                                                *
...
let name_of_id id = string_of_id {str=default_sname;idid=id}
let name_of_iv iv = string_of_id {str=(snd iv);idid=(fst iv)}
...
(* Model Printing Stuff *)
let get_assignment model expr = Z3.Model.get_const_interp_e model expr
let get_assignments model = List.map (get_assignment model)
let print_declarations model =
  List.map print_endline (List.map Z3.FuncDecl.to_string (Z3.Model.get_decls model))
let print_names model =
  List.map print_endline
    (List.map (fun x -> Z3.Symbol.to_string (Z3.FuncDecl.get_name x)) (Z3.Model.get_decls model))

(* list based *)
let assignments_list model =
  ...
  let mk_pair decl =
    ...
    | Some func_interp ->
       Some 
         (Z3.Symbol.to_string (Z3.FuncDecl.get_name decl) ,
          Z3.Expr.to_string func_interp)
  in
  List.filter_map mk_pair decls

let print_asignments_list model =
  let ls = assignments_list model in
  List.iter (fun (x , y) -> Printf.printf "%s := %s\n" x y) ls

let filter_print_asignments_list sigma filter =
  let _ = check_sat_sigma sigma in
  let model = get_model () in
  let ls = assignments_list model in
  List.iter
    (fun (x , y) ->
      if List.mem x filter then Printf.printf "%s := %s\n" x y) ls

(* map based *)
module StringMap = Map.Make(String)
let assignments_map model =
  let decls = Z3.Model.get_const_decls model in
  let mk_pair acc decl =
    match Z3.Model.get_const_interp model decl with
    | None -> acc
    | Some func_interp ->
       let name = Z3.Symbol.to_string (Z3.FuncDecl.get_name decl) in
       let value = Z3.Expr.to_string func_interp in
       StringMap.add name value acc
  in
  List.fold_left mk_pair StringMap.empty decls

let print_asignments_map model  =
  let map = assignments_map model in
  StringMap.iter (fun x y -> Printf.printf "%s := %s\n" x y) map

let filter_print_asignments_map sigma filter =
  let _ = check_sat_sigma sigma in
  let model = get_model () in
  let map = assignments_map model in
  List.iter
