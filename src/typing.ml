...
let rec normalise_val mode v=
  ...
  | FunVal (f, ft, p, pt, e) ->
     FunVal (f, normalise_type mode ft, p, normalise_type mode pt, normalise_exp mode e)
  | AbsCon (i, t, s) -> AbsCon (i, normalise_type mode t, s)
  | AbsFun (i, t1, t2, s, ls) ->
     AbsFun (i, normalise_type mode t1, normalise_type mode t2, s, ls)
...
let rec type_of_val tenv v =
  ...
  | FunVal (f, ft, p, pt, e1) ->
      ...
      fun_t
  | AbsCon (uid, t, s) -> t
  | AbsFun (uid, t1, t2, s, _) -> Type.FunT ([t1], t2)
