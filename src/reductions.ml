...
(*** dummy dependency tree for when we decide to add it ***)
...
let focus {e = {ecxt = ecxt0; e = e0}} =
  ...
  | ArithExp (op, [], p) -> None
  | ArithExp (op, e1 :: es, p) ->
      Some { e = {ecxt = ArithECxt (op, [], es, p) :: ecxt0; e = e1 }}
  | AppExp (e1, e2, p) ->
      Some { e = {ecxt = AppOpECxt (e2, p) :: ecxt0; e = e1 }}
  | CondExp (e1, e2, e3, p) ->
      Some { e = {ecxt = CondECxt (e2, e3, p) :: ecxt0; e = e1 }}
  | LetExp (i, it, e1, e2, p) ->
      Some { e = {ecxt = LetECxt (i, it, e2, p) :: ecxt0; e = e1 }}
  | LetTuple (is_ts, e1, e2, p) ->
      Some { e = {ecxt = LetTupleECxt (is_ts, e2, p) :: ecxt0; e = e1 }}
  | SeqExp (e1, e2, p) ->
      Some { e = {ecxt = SeqECxt (e2, p) :: ecxt0; e = e1 }}
  | TupleExp (es, p) -> begin
    match es with
    | [] ->
        raise (Error.RuntimeE (get_lex_pos e0, "tuple expression with zero elements"))
    | e1 :: rest ->
        Some { e = {ecxt = TupleECxt ([], rest, p) :: ecxt0; e = e1 }}
    end
  | BotExp p -> None
  | TupleProjExp (e1, i, j, p) ->
        Some { e = {ecxt = TupleProjECxt (i, j, p) :: ecxt0; e = e1 }}
  | TupleUpdExp (e1, i, j, e2, p) -> 
        Some { e = {ecxt = TupleFstUpdECxt (i, j, e2, p) :: ecxt0; e = e1 }}
...
let value_to_prop v =
  ...
  | AbsCon (id,typ,name) ->
     begin
       match typ with
       | BoolT -> sbool_of_id_var (id,name) , Some id
       | IntT -> sint_of_id_var (id,name) , Some id
       | _ -> failwith "const to prop: encountered unexpected type"
     end
  | AbsFun _ -> failwith "const to prop: function alphas should not be symbolic constants"
...
let reduce_arith op vs p sigma0 =
  ...
  let symbolic_binop op i1 i2 typ =
    ...
    in
    Some (ValExp (i3, p), (new_sigma , DummyDT))
  in
...
let reduce { e ={ecxt = ecxt0; e = e0}} bound0 sigma0 =
  match e0 with
  | BotExp _ -> None :: []
  | ValExp (v, _) ->
     begin
       ...
       | ArithECxt (op, vs, [], p) :: ec_rest ->
          begin
            ...
            | Some (e_new, sigma11) -> Some ({ e = {ecxt = ec_rest; e = e_new }}, bound0, sigma11) :: []
            | None -> None :: []
          end
       | ArithECxt (op, vs, e1 :: e_rest, p) :: ec_rest ->
          Some ({ e = {ecxt = (ArithECxt (op, vs @ [v], e_rest, p)) :: ec_rest; e = e1}},
                bound0, sigma0) :: []
       | AppOpECxt (e1, p) :: ec_rest ->
          Some ({ e = {ecxt = (AppRandECxt (v, p)) :: ec_rest; e = e1}}, bound0, sigma0) :: []
       | AppRandECxt (v1, p) :: ec_rest  ->
          begin
            match (reduce_app v1 v bound0) with 
            | None -> None :: []
            | Some (e1, bound1) -> Some ({ e = {ecxt = ec_rest; e = e1}}, bound1, sigma0) :: [] 
          end
       | CondECxt (e1, e2, p) :: ec_rest  ->
          begin
            match v with
            | ConstVal (BoolConst true) ->
               Some ({ e = {ecxt = ec_rest; e = e1}}, bound0, sigma0) :: []
            | ConstVal (BoolConst false) ->
               Some ({ e = {ecxt = ec_rest; e = e2}}, bound0, sigma0) :: []
            | AbsCon (id, BoolT, name) ->
               ...
               let new_conf e' sigma' =
                 Some ({ e = {ecxt = ec_rest; e = e'}}, bound0, sigma') in
       ...
       | LetECxt (i, it, e1, p) :: ec_rest  ->
          Some ({ e = {ecxt = ec_rest; e = (beta_subst e1 i v)}}, bound0, sigma0) :: []
       | LetTupleECxt (is_ts, e1, p) :: ec_rest  ->
          begin
           match v with
           | TupleVal vs ->
              if (List.length is_ts <> List.length vs) then None :: []
              else Some ({ e = {ecxt = ec_rest; e = (multi_beta_subst e1 (fst (List.split is_ts)) vs)}}, bound0, sigma0) :: []
           | _ -> None :: []
         end
       | SeqECxt (e1, p) :: ec_rest  ->
          Some ({ e = {ecxt = ec_rest; e = e1}}, bound0, sigma0) :: []
       | TupleECxt (vs, [], p) :: ec_rest ->
          Some ({ e = {ecxt = ec_rest; e = ValExp (TupleVal (vs @ [v]), p)}},
                bound0, sigma0) :: []
       | TupleECxt (vs, e1 :: e_rest, p) :: ec_rest ->
          Some ({ e = {ecxt = (TupleECxt (vs @ [v], e_rest, p)) :: ec_rest; e = e1}},
                bound0, sigma0) :: []
       | TupleProjECxt (i, j, p) :: ec_rest ->
          begin
            match v with
            | TupleVal vs ->
               Some ({ e = {ecxt = ec_rest; e = ValExp (List.nth vs i, None) }},
                     bound0, sigma0) :: []
            | _ -> failwith "reduce error: unexpected pattern with TupleProjECxt"
          end
       | TupleFstUpdECxt (i, j, e2, p) :: ec_rest ->
          Some ({ e = {ecxt = (TupleSndUpdECxt (v, i, j, p)) :: ec_rest; e = e2}},
                bound0, sigma0) :: []
          ...
          begin
            ...
            | TupleVal vs ->
                (let newvs = List.mapi (fun ii x -> if ii = i then v else x) vs in
                 Some ({ e = {ecxt = ec_rest; e = ValExp (TupleVal newvs, None)}},
                       bound0, sigma0) :: [])
...
(*** helper functions ***)
let string_of_red_conf {e} = Printf.sprintf "{ e = %s }" (string_of_exp (unfocus e))
